...
### 6. 请求日志与统计
- **完整请求日志**：记录每一个请求的完整信息
  - 时间戳、渠道名、Key（脱敏显示）、model
  - prompt_tokens、completion_tokens、total_tokens
  - 响应时间(ms)、HTTP状态码、是否成功
  - 错误信息（失败时记录具体错误）
  - 请求来源IP
- **Key 使用统计**：
  - 每个 Key 的总请求次数、成功次数、失败次数
  - 成功率、平均响应时间
  - 累计 token 用量（prompt + completion）
  - 最近一次使用时间
- **渠道统计**：
  - 每个渠道的总请求量、成功率
  - 可用 Key 数量 vs 总 Key 数量
  - 平均响应时间
- **全局统计**：
  - 总请求量、成功率、平均响应时间
  - 按 model 分类的使用统计
  - 每小时/每天的请求量趋势
- **日志导出**：支持导出 CSV
- **日志保留策略**：可配置保留天数，自动清理过期日志

### 7. 管理 UI（Gemini 设计）
- 渠道列表页：显示所有渠道、状态、Key数量
- 渠道详情页：显示渠道下的所有 Key、状态、使用统计
- Key 批量添加：文本框粘贴，支持逗号/换行分隔
- Dashboard：全局统计（总请求量、成功率、avg响应时间）、按model分类统计、每小时/天趋势图、Key健康概览
- 渠道详情页：渠道级统计（请求量、成功率、可用Key数）、Key使用统计表格
- 请求日志页：详细请求记录表格，可按渠道/Key/model/时间筛选，支持导出CSV
- 设置页：代理配置、检测间隔、重试次数、日志保留天数
